...
module.exports = {
  ...
  getTemplatesJSON({ username }) {
    ...
    const pathPromise = paths.getTemplatesPath({ username }).then(templatesPath => {
      templatesJSONPath = templatesPath;
      return templatesPath;
    });

    return pathPromise.then(path => files.readJSON({ path })).catch(err => {
      debug(err);
  ...
  find(params, callback) {
    ...
    const templatePromises = templateIndexPromise.then(templateIndex => {
      return _.map(templateIndex, ({ templateId }) => {
        return paths.getTemplatePath({ templateId, fileDataUrl }).then(path => files.readJSON({ path }));
      });
    });
  ...
  get(guideId, params, callback) {
    ...
    const filteredTemplateSummaries = usernamePromise.then(username => this.getTemplatesJSON({ username })).then(filterByGuideId);

    const templatePromises = Q.all([filteredTemplateSummaries, usernamePromise]).then(([filteredTemplates, username]) => {
