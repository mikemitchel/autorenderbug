...
module.exports = {
  ...
  get(templateId, params, callback) {
    ...
    const templateSummaryPromise = usernamePromise.then(username => templates.getTemplatesJSON({ username })).then(templatesData => this.filterTemplatesByTemplateId({ templatesData, templateId }));

    Q.all([templateSummaryPromise, usernamePromise]).then(([{ guideId, templateId }, username]) => paths.getTemplatePath({
      guideId,
      templateId,
      username
    })).then(templatePath => files.readJSON({ path: templatePath })).then(data => this.successHandler({
  ...
  create(data, params, callback) {
    ...
    const templateDataPromise = usernamePromise.then(username => templates.getTemplatesJSON({ username })).then(templatesData => this.getNextTemplateId({ templatesData })).then(templateId => _.assign(data, {
      templateId,
      createdAt: currentTime,
      updatedAt: currentTime
    }));

    const writeTemplatePromise = Q.all([templateDataPromise, usernamePromise]).then(([{ guideId, templateId }, username]) => paths.getTemplatePath({
      guideId,
      templateId,
      username
    })).then(path => files.writeJSON({ path, data }));

    const templatesPathPromise = usernamePromise.then(username => paths.getTemplatesPath({ username }));

    const writeSummaryPromise = Q.all([templatesPathPromise, templateDataPromise]).then(([path, templateData]) => files.mergeJSON({
      path,
      data: _.pick(templateData, this.summaryFields)
    }));

    Q.all([writeTemplatePromise, writeSummaryPromise]).then(([data]) => this.successHandler({
  ...
  update(templateId, data, params, callback) {
    ...
    const writeTemplatePromise = usernamePromise.then(username => paths.getTemplatePath({
      guideId: data.guideId,
      templateId: data.templateId,
      username
    })).then(path => files.writeJSON({ path, data }));

    const writeSummaryPromise = usernamePromise.then(username => paths.getTemplatesPath({ username })).then(path => files.mergeJSON({
      path,
