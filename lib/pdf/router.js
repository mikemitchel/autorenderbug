...
const storageRouter = new Router().use(user.middleware).get('/:templateId', async (req, res) => {
  ...
  storage.saveTemplatePdf(username, templateId, filepath).then(() => unlink(filepath)).then(() => res.json({ ok: true })).catch(error => errorRes(res, error));
  ...
  res.sendFile(storage.getTemporaryFilepath(pdfId), error => {
    ...
    storage.deleteTemporaryFilepath(pdfId).catch(error => console.error(error));
  });
});

const overlayRouter = new Router().post('/apply', (req, res) => {
  const { pdfId, overlay } = req.body;
  storage.getTemporaryFilepathForId(pdfId).then(({ filepath, pdfId: newPdfId }) => {
    return overlayer.forkWithOverlay(filepath, overlay).then(() => res.json({ ok: true, pdfId: newPdfId }));
...
const router = new Router().use('/storage', storageRouter).use('/overlay', overlayRouter);
